...
// - قفل بودن بازی تا وارد کردن کد تیکت معتبر
...
let ticketAccepted = false; // فقط اگر true شد بازی اجازه حرکت دارد
...
function move(direction) {
  if (gameStopped || !ticketAccepted) return; // تا تیکت اوکی نشه، حرکتی نیست
  if (isMoving) return;
  isMoving = true;
...
document.addEventListener("keydown", (e) => {
  if (gameStopped || !ticketAccepted) return;
...
// ---------- منطق تیکت ----------
// ticket.js باید قبل از این فایل لود شود و متغیر VALID_TICKETS را تعریف کند
...
ticketSubmit.addEventListener("click", () => {
  ...
  if (used.includes(code)) {
    ticketError.textContent = "کد تیکت اشتباه است"
    ticketError.classList.remove("hidden");
    return;
  }

  // اگر کد معتبر باشد
  if (Array.isArray(VALID_TICKETS) && VALID_TICKETS.includes(code)) {
    ticketAccepted = true;
...
initGridBackground();        // فقط گرید را می‌سازد
ticketModal.style.display = "flex"; // اول از همه مودال تیکت باز باشد

document.getElementById("ticket-help").addEventListener("click", () => {
  Telegram.WebApp.sendData("HELP_TICKET");
});